<template>
	<view>
		<!-- 联系人列表(正常显示) -->
		<view v-if="type == 'addressList'" class="flex flex-direction-row flex-vc flex-jsb width-750 height-120 pd-lr30" v-for="(item, index) in friendsList" :key="index">
		...
		<!-- 创建群聊独有的样式 -->
		<view v-if="type == 'createChatRoom'" class="uni-list" v-for="(item, index) in friendsList" :key="index">
